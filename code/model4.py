...
def calculate_hydrological_metrics(observed, predicted):
    """수문학 평가지표 (ME, RMSE, PBIAS, NSE, VE, KGE, NPE)를 계산합니다."""
    ...
    if len(observed) == 0:
        return {
            ...
            'NSE': np.nan, 'VE': np.nan, 'KGE': np.nan, 'NPE': np.nan
        }
    ...
    ve = 1 - np.sum(np.abs(predicted - observed)) / np.sum(observed) if np.sum(observed) != 0 else np.nan
    ...
    npe = np.mean(np.abs(predicted - observed)) / mean_obs if mean_obs != 0 else np.nan

    return {
        'ME': me, 'RMSE': rmse, 'PBIAS': pbias,
        'NSE': nse, 'VE': ve, 'KGE': kge, 'NPE': npe
    }
...
if df_flow_x is None or df_rain is None or df_suwi is None or df_flow_y is None:
    ...
    # 4. BiLSTM 모델 구축
    def build_bilstm_model(input_shape, future_sequence_length, num_output_targets, lstm_units=100, dropout=0.2):  # units 증가, dropout 감소
        inputs = Input(shape=input_shape)
        
        # 더 깊은 네트워크 구조
        x = Bidirectional(LSTM(lstm_units, return_sequences=True, 
                              kernel_regularizer=regularizers.l2(0.005)))(inputs)  # L2 정규화 감소
        x = LayerNormalization()(x)  # 레이어 정규화 추가
        x = Dropout(dropout)(x)
        ...
        outputs = TimeDistributed(Dense(num_output_targets))(x)
    ...
    model.compile(
        ...
        loss=tf.keras.losses.Huber(),  # 'huber_loss' 대신 tf.keras.losses.Huber() 사용
        metrics=['mae', 'mse']
    )
    ...
    metrics_data = {
        'Metric': ['MAE', 'MSE', 'R2', 'ME', 'RMSE', 'PBIAS', 'NSE', 'VE', 'KGE', 'NPE'],
        'Value': [
    ...
    def evaluate_model_with_test_files(model, scaler_flow, scaler_rain, scaler_suwi, scaler_y, 
        ...
        print(f"{'NPE':<15} | {hydrological_metrics['NPE']:<10.4f}")
        print("-" * 30)
